...
# create workload identity pool & bind service account to it.
...
WORKLOAD_IDENTITY_POOL_ID=$(
	gcloud iam workload-identity-pools describe "${WORKLOAD_IDENTITY_POOL}" \
		...
		--location="global" \
		--format="value(name)"
	projects/750576968/locations/global/workloadIdentityPools/github-actions
)
...
echo -e ">> pool created with provider '${WORKLOAD_IDENITY_PROVIDER}'"
